...
# -*- coding: utf-8 -*-
"""
오답 누적 원장 빌더 v3.1 — 닫힌 루프(상태머신) + FSRS-6 스케줄링 + 과목 별칭 정규화 · 2026-08-20
...
[FSRS 설정 — 왜 이렇게 잡았나]
  ...
  maximum_interval=60                      시험(2026-10-31) 대비, 60일 넘겨 밀지 않는다
...
FSRS_MAX_INTERVAL = 60                            # 시험 2026-10-31 대비 상한(일)
...
# ─────────────────────────────────────────────────────────────────────────────
# v3.1 — 과목 별칭 정규화
#   퀴즈 생성기가 회차마다 과목을 다르게 적는다(단원명으로 적거나, 접두어 '부동산'을 빼거나).
#   집계 축이 쪼개지면 "민법이 약점 1위"라는 사실이 표에서 사라지므로 대표명으로 접는다.
#   ⚠️ subject(집계 축)에만 적용한다. conceptKey/concept/aliases(매칭 키)는 무관하다.
# ─────────────────────────────────────────────────────────────────────────────
SUBJECT_ALIAS = {"민법총칙": "민법", "물권법": "민법", "계약법": "민법",
                 "민법 물권법": "민법", "민법 용익·담보물권": "민법",
                 "민법 계약·특별법": "민법", "민사특별법": "민법",  # 1차 과목명 "민법 및 민사특별법" 기준
                 "공법": "부동산공법", "공시법": "부동산공시법", "세법": "부동산세법",
                 "중개사법": "공인중개사법", "학개론": "부동산학개론"}
...
def main():
    ...
    DUE_TABLE_ROWS = 20
    L = []
    L.append(f"# 📒 공인중개사 오답 누적 원장 v3.1 · FSRS-6 스케줄링  ·  갱신 {out['generatedAt']}")
    L.append("")
    ...
    L.append("")
    L.append("_과목명은 대표명으로 통일해 집계한다(v3.1) — 민법총칙·물권법·계약법 → 민법, 공법 → 부동산공법 등._")
    L.append("")
    L.append("| 과목 | 오답 |")
    L.append("|---|---|")
