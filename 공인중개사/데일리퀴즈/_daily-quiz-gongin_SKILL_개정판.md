...
description: 평일(월~금) 아침, 최근 7일 노트 + 1~6개월 전 장기복습 노트로 40문항 v3 자동 생성 — 장기복습 로테이션(⏪)·원장 듀큐 재도전·카톡 푸시 페이로드·정본 템플릿·계산 검산
---

> ⚙️ 설치: 이 본문은 2026-08-20 v3 기준. 예약작업 프롬프트·설치 스킬 모두 동일 내용으로 이미 반영됨(백업용 사본).

평일(월~금) 아침에 실행되는 "데일리 문제지 자동 생성" 작업이다. 아래를 순서대로 수행하라.

**[실행 원칙] 이 지침은 실행 모델의 재량을 최소화하도록 쿼터·수열·스크립트로 고정되어 있다. 단계를 건너뛰거나 "비슷하게" 단순화하지 말고, 명시된 숫자와 절차를 그대로 따른다. 품질의 핵심 4가지: ① 노트 전체 정독 ② 과거 문제 회피 ③ 한 끗 차이 보기 ④ 검증 통과까지 반복. ⑤ 장기복습(⏪)은 로테이션 로그 기준으로 기계적으로 선정.**
...
- 나이 버킷과 문항 수:
  ...
  | B2 | 45~90일 전 | 3문 | 10문 |
  | B3 | 90일+ 전 | 4문 | 12~15문 |
  - 기본 모드 합계 10문 고정. 전일 모드 합계 = 40 − 재도전 수(32~35문). 버킷에 후보가 없으면 그 몫은 더 오래된 버킷으로 이월한다.
...
- 표기: 문제문 맨 앞 `⏪ `, src 끝에 `(장기복습·N일전)` (N = 오늘 − 노트 파일명 날짜). conceptKey 규칙은 신규와 동일. "확인 필요/⚠️" 출제 금지와 §1-2 과거 출제 회피는 장기복습에도 그대로 적용(재도전이 아니므로 의도적 반복 불가).
- 생성 완료 후 로그 갱신: 선정된 각 노트에 `last=오늘, n+=1`을 기록해 저장한다.

## 2. 출제 계획표 작성 (문항 작성 전 필수 — 건너뛰기 금지)
40행 계획표: `# | 소스(신규/장기/재도전) | 과목 | 출처 소제목 | conceptKey | 개념 + 함정 설계(무엇을 한 끗 바꿀지) | type | 정답idx | 🔁여부`
### 2-1. 과목 배분
- 40문항을 발견된 과목 수로 고르게(차이 ≤1, 재도전 몰린 과목만 +2 허용). **QUESTIONS 배열에서 같은 과목 문항은 연속 배치.**
- 소스 구성(합 40): 재도전 5~8 · 장기복습 10 · 신규 = 40 − 10 − 재도전 수. [전일 모드] 재도전 5~8 · 장기복습 = 40 − 재도전 수. 유형 쿼터(§2-2)는 소스와 무관하게 전체 40 기준으로 유지한다.
...
- 교체할 것 4가지만:
  ...
  2. `{{META_LINE}}` → "2026년 M월 D일 (요일) · 실전형 40문항 · 재도전 N(🔁) · 장기복습 N(⏪)"
  3. `{{TAGS_HTML}}` → `<span class="tag">🔥 심화</span><span class="tag">📝 실전 4지선다</span><span class="tag">🔁 오답 재도전 N</span><span class="tag">⏪ 장기복습 N</span><span class="tag">🎯 정답분산</span>` + 오늘 과목명 태그들
  4. `/*__QUESTIONS_START__*/`와 `/*__QUESTIONS_END__*/` **사이**에 40문항 배열 원소 삽입
- **그 외 JS·CSS(buildPayload·downloadResult·클립보드 폴백·보기소거 눈동자(👁) UI 포함) 수정 절대 금지.** 눈동자 소거 버튼은 순수 시각용 학습보조로 채점·payload에 영향이 없으니 그대로 둔다.
- 저장: `$QD/{오늘날짜}.html`
...
const lr=Q.filter(q=>q.q.includes("⏪")).length;
ok(lr===10||(lr>=32&&lr<=35),"장기복습(⏪) 10(기본) 또는 32~35(전일) (현재 "+lr+")");
ok(Q.filter(q=>q.q.includes("⏪")).every(q=>/\(장기복습·\d+일전\)/.test(q.src)),"장기복습 표기(⏪·src) 일관");
...
## 5.5 카톡 푸시 페이로드 생성 (검증 통과 후 — v3 신설)
- `mkdir -p "$QD/_push"` 후 `$QD/_push/{오늘날짜}.json` 을 아래 스키마 **그대로** 생성한다:
  ```json
  {"date":"YYYY-MM-DD","quizFile":"YYYY-MM-DD.html",
   "counts":{"신규":0,"장기복습":0,"재도전":0},
   "longRevNotes":["노트 제목 (N일전)"],
   "dueBacklog":0,
   "ox":[{"q":"...","a":"O","exp":"..."}]}
  ```
- 규칙: `longRevNotes` 최대 3개. `dueBacklog` = 원장 dueQueue 길이(원장 없으면 null). `ox`는 정확히 3개 — **오늘 퀴즈 40문에 없는 새 OX**를 장기복습 노트 개념에서 만든다. q ≤ 100자, exp ≤ 80자, a는 "O" 또는 "X"(3문 중 O 1~2개), 모든 문자열에 큰따옴표(")와 백틱 금지.
...
## 6. 사용자에게 전달
- 완성된 .html을 present_files로 보여준다. 어떤 과목/개념에서 냈는지·재도전 수·장기복습 노트명(N일전)을 두세 문장으로 요약. 장황한 설명 금지. 모든 문구 한국어, 영어 용어는 한글 병기. 추천 질문 3개 포맷은 생략.
