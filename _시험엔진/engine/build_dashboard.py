...
1. 배치 이동: 공인중개사/claude_ox_오답/_ledger/ → _시험엔진/engine/ (엔진 단일화).
2. --exam 인자 신설. 시험 폴더(dir)·시험일(exam_date)·라벨(exam_label)·결과 파일 패턴을
   exams.json에서 읽는다(하드코딩 EXAM_DATE/EXAM_LABEL 및 "공인중개사" 접두어 제거).
3. --root 의미 변경: (구) 공인중개사 폴더 → (신) velog-posts 루트. 미지정 시 자동 탐지.
4. NOTE_AGE_DAYS 14 → 7. 데일리퀴즈의 장기복습 창이 -7일로 바뀐 것에 정합
   (exams.json daily.new_note_window_days / blank_review_window_days = 7).

배치 위치(가정):
    velog-posts/_시험엔진/engine/build_dashboard.py

사용법:
    python3 build_dashboard.py --exam gongin              # 데일리퀴즈/_오늘의복습.html 갱신
    python3 build_dashboard.py --exam gongin --root <velog-posts 경로>   # 루트 수동 지정
    python3 build_dashboard.py --exam gongin --out  <폴더>  # 프로덕션 대신 스테이징에 출력
...
# 장기복습 후보가 되는 최소 나이(파일명 날짜 기준).
#   2026-09-02: 14 → 7. 데일리퀴즈가 "최근 7일 노트 = 신규 / 그 이전 = 장기복습"으로
#   창을 바꿨는데(exams.json daily.new_note_window_days=7) 여기만 14로 남아 있으면
#   8~13일 된 노트가 '신규도 아니고 커버리지 분모도 아닌' 사각지대가 된다.
NOTE_AGE_DAYS = 7
...
def load_exams():
    try:
        with open(EXAMS_JSON, "r", encoding="utf-8") as fp:
            return json.load(fp)
    except Exception as exc:
        raise BuildError("exams.json을 읽지 못했습니다: %s (%s)" % (EXAMS_JSON, exc))


def exam_config(cfg, key):
    exams = cfg.get("exams") or {}
    if key not in exams:
        raise BuildError("모르는 시험 키 '%s'. 가능한 값: %s" % (key, " | ".join(exams)))
    return exams[key]


def exam_day(exam_cfg):
    """exams.json의 exam_date(YYYY-MM-DD)를 date로. 없거나 깨지면 None."""
    found = parse_date(exam_cfg.get("exam_date"))
    return found


def exam_label(exam_cfg):
    """푸터 라벨. exam_label이 없으면 exam_date에서 'M/D'로 만든다."""
    label = exam_cfg.get("exam_label")
    if label:
        return str(label)
    day = exam_day(exam_cfg)
    return "%d/%d" % (day.month, day.day) if day else "시험일 미정"
...
def resolve_paths(cfg, exam_cfg, root_opt):
    """스크립트가 <root>/_시험엔진/engine/ 에 있다고 보고 프로덕션 경로를 유도한다.

    (v4.0) --root는 이제 '공인중개사 폴더'가 아니라 **velog-posts 루트**다.
    엔진이 시험 밖으로 나왔으므로 기준점도 루트로 올라간다.
    """
    paths_cfg = cfg.get("_paths") or {}
    if root_opt:
        root = os.path.abspath(os.path.expanduser(root_opt))
    else:
        root = os.path.dirname(os.path.dirname(HERE))     # engine → _시험엔진 → velog-posts
    exam_dir = os.path.join(root, exam_cfg["dir"])         # 예: <root>/공인중개사
    notes_dir = os.path.join(root, exam_cfg.get("notes_dir") or exam_cfg["dir"])
    base = os.path.join(exam_dir, paths_cfg.get("ledger_dir") or "claude_ox_오답")
    ledger_dir = os.path.join(base, paths_cfg.get("ledger_subdir") or "_ledger")
    quiz_dir = os.path.join(exam_dir, (paths_cfg.get("quiz_dirs") or {}).get("daily") or "데일리퀴즈")
    return {
        "here": HERE,
        "root": root,
...
def collect_submission_dates(base_dir, result_glob):
    """_raw·_inbox의 결과 JSON에서 제출 날짜 집합을 만든다(YYYY-MM-DD 문자열).

    (v4.0) 파일명 필터를 하드코딩 "공인중개사" 접두어 → exams.json의 result_glob으로 바꿨다.
    build_ledger.py가 수거하는 파일 집합과 정확히 같은 기준을 쓴다.
    """
    dates = set()
    for sub in ("_raw", "_inbox"):
        ...
        for raw_name in names:
            ...
            if not name.lower().endswith(".json"):
                continue
            if not fnmatch.fnmatch(name, result_glob):
                continue
...
def count_old_notes(notes_dir, cutoff):
    """노트 폴더 최상위의 20*.md 중 파일명 날짜가 cutoff 이하인 개수. 폴더 없으면 None."""
    ...
    for raw_name in names:
        ...
        if found is None or found > cutoff:
            continue
        if os.path.isfile(os.path.join(notes_dir, raw_name)):   # 하위 폴더 제외
            total += 1
    return total
...
def render_html(ctx):
    ...
    add('<div class="eyebrow">%s · 데일리 복습</div>' % esc(ctx["exam_name"]))
    add("<h1>오늘의 복습</h1>")
    ...
    dday = ctx["dday"]
    label = ctx["exam_label"]
    if dday is None:
        add('<span><span class="dday">시험일 미정</span> (%s)</span>' % esc(label))
    elif dday >= 0:
        add('<span><span class="dday">D-%d</span> (%s)</span>' % (dday, esc(label)))
    else:
        add('<span><span class="dday">시험 종료</span> (%s)</span>' % esc(label))
    add("</footer>")
...
def build(paths, exam_cfg, today, out_dir):
    ...
    # 2) 제출 이력·스트릭
    submitted = collect_submission_dates(paths["base"], exam_cfg["result_glob"])
    ...
    candidates = count_old_notes(paths["notes_dir"], cutoff)
    if candidates:
    ...
    context = {
        ...
        "dday": (e_day - today).days if e_day else None,
        "exam_name": exam_cfg.get("name") or "",
        "exam_label": exam_label(exam_cfg),
...
def run(argv):
    ...
    parser.add_argument("--exam", default="gongin",
                        help="시험 키(exams.json). 기본 gongin — dashboard:true인 유일한 시험이다.")
    parser.add_argument("--root", default=None,
                        help="velog-posts 루트 경로(기본: 스크립트 위치에서 자동 탐지)")
    parser.add_argument("--out", default=None,
                        help="출력 폴더(기본: 데일리퀴즈/, 히스토리는 _ledger/)")
    args = parser.parse_args(argv)

    cfg = load_exams()
    exam_cfg = exam_config(cfg, args.exam)
    if not exam_cfg.get("dashboard"):
        print("ℹ️  '%s'은(는) exams.json에서 dashboard:false입니다 — 그래도 생성합니다."
              % args.exam)

    paths = resolve_paths(cfg, exam_cfg, args.root)
    if not os.path.isdir(paths["exam_dir"]):
        raise BuildError("시험 폴더를 찾지 못했습니다: %s" % paths["exam_dir"])
    ...
    html, history, summary = build(paths, exam_cfg, today, out_dir)
