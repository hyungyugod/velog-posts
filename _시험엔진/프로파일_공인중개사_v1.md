...
- **신규 원천 규칙**: `find "$ROOT/공인중개사" "$ROOT/공인중개사/백지복습" -maxdepth 1 -name "20*.md" -mtime -7` (그 외 하위 폴더 제외 — v1.1). 백지복습 보고서는 "남은 구멍"·"교정된 개념" 표가 최우선 소재이고 "내가 쓴 오답" 열은 오답 보기(한 끗 차이) 재료로 쓴다. front matter `status: 닫힘`인 보고서와 ⚠️/확인 필요 섹션은 출제 금지.
