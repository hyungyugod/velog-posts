...
> v1.1 (2026-08-25): 신규 원천에 `백지복습/` 추가 — 백지복습 보고서(코어엔진 v3.3 §4-3 산출)를 데일리퀴즈 신규 원천에 편입. 파일명은 v1 유지(스케줄러 참조 보호).
...
- **신규 원천 규칙**: `find "$ROOT/공인중개사" "$ROOT/공인중개사/백지복습" -maxdepth 1 -name "20*.md" -mtime -7` (그 외 하위 폴더 제외 — v1.1). 백지복습 보고서는 "남은 구멍"·"교정된 개념" 표가 최우선 소재이고 "내가 쓴 오답" 열은 오답 보기(한 끗 차이) 재료로 쓴다. front matter `status: 닫힘`인 보고서와 ⚠️/확인 필요 섹션은 출제 금지. **★ 가중(v1.2)**: 노트 안 `★`/`★★`(또는 `!`/`!!`) 표시 항목은 신규 문항의 우선 소재로 삼는다 — 과목 균형 내에서 ★★ 최우선.
